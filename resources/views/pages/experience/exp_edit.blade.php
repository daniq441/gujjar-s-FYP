...
@section('content')
    <section class="container bg-white border my-2">
        ...
        @if(@isset($bg_color))
        <form action="/experience-update/{{ $image}}/{{$color}}/{{ $bg_color }}/{{ $templatePath }}/{{ $exp_data->id }}" method="POST">
        @else
        <form action="/experience-update/{{ $image}}/{{$color}}/{{ $templatePath }}/{{ $exp_data->id }}" method="POST">
        @endif
            @csrf
